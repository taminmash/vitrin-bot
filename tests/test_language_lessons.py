...
def message_update(text):
    return SimpleNamespace(message=SimpleNamespace(text=text, reply_text=AsyncMock()), effective_user=SimpleNamespace(id=123))
...
class LessonHandlerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_like_persistence(self, save):
    ...
    @patch("handlers.language_lessons.save_language_lesson_reaction")
    async def test_dislike_and_reaction_switching(self, save):
    ...
    @patch("handlers.language_lessons.save_language_lesson_reaction")
    async def test_duplicate_reaction_is_delegated_to_upsert(self, save):
        context = SimpleNamespace(user_data={})
        for _ in range(2):
    ...
    @patch("handlers.language_lessons.save_language_lesson_comment")
    async def test_successful_comment_clears_state(self, save):
        context = SimpleNamespace(user_data={FEEDBACK_STATE_KEY: {"action": "comment", "level": "a1", "lesson_number": 1}})
    ...
    @patch("handlers.language_lessons.save_language_lesson_report")
    async def test_successful_report_clears_state(self, save):
